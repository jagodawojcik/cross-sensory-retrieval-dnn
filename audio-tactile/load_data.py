...
class CustomDataSet(Dataset):
    def __init__(self, audio, tactile, labels):
        self.audio = audio
    ...
    def __len__(self):
        ...
        assert len(self.tactile) == len(self.labels), "Mismatched tactile and label lengths."
        return count
...
def fetch_data():
    ...
    ## Object numbers used in the model
#     object_numbers = [1, 2, 3, 4, 13, 17, 18, 25, 29, 30, 33, 49, 50, 66, 67, 68, 71, 83, 89, 100]
    ...
    for object_number in object_numbers:
        folder_dir = f"/kaggle/input/audio-tactile-data/audio_split/train/{object_number}"
#         count = 0
...
#                 count += 1
    ...
    for object_number in object_numbers:
        folder_dir = f"/kaggle/input/audio-tactile-data/audio_split/test/{object_number}"
#         count = 0
        for audio_files in os.listdir(folder_dir):
#             if count >= 20:
#                 break
...
#                 count += 1
    ...
    for object_number in object_numbers:
        folder_dir = f"/kaggle/input/audio-tactile-data/tactile/train/{object_number}"
#         count = 0
...
#                 count += 1
    ...
    for object_number in object_numbers:
        folder_dir = f"/kaggle/input/audio-tactile-data/tactile/test/{object_number}"
#         count = 0
        for images in os.listdir(folder_dir):
#             if count >= 20:
#                 break
...
#                 count += 1
    ...
    return audio_train, audio_test, tactile_train, tactile_test, label_train, label_test



def get_loader(batch_size):
    audio_train, audio_test, tactile_train, tactile_test, label_train, label_test = fetch_data()
...
#     labels = [1, 2, 3, 4, 13, 17, 18, 25, 29, 30, 33, 49, 50, 66, 67, 68, 71, 83, 89, 100]
    ...
    dataset = {x: CustomDataSet(audio=audio[x], tactile=tactile[x], labels=labels[x]) 
               for x in ['train', 'test']}
